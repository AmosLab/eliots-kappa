...
load_from_dir <- function(dirpath=".", pat="*.csv$", header=FALSE, delimiter=",", colTypes=c("numeric", "numeric", "character", "character"), parseDates=c(T,T,F,F), dateFmt="ms") {
  ...
  for (f in files){
    print(f)
    d <- load_from_file(file.path(dirpath,f), header=header, delimiter=delimiter, colTypes=colTypes, parseDates=parseDates, dateFmt=dateFmt)
